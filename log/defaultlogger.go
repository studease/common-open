...
// Init me class
func (me *DefaultLogger) Init(n level.Level, calldepth int) *DefaultLogger {
	me.level = n
...
func NewDefaultLogger(out io.Writer, scope string, n level.Level, calldepth int) *DefaultLogger {
	return new(DefaultLogger).Init(n, calldepth).
		WithTrace(log.New(os.Stdout, fmt.Sprintf("[TRACE] %s ", scope), log.LstdFlags|log.Lshortfile)).
		WithDebug(log.New(out, fmt.Sprintf("[DEBUG] %s ", scope), log.LstdFlags|log.Lshortfile)).
		WithInfo(log.New(out, fmt.Sprintf("[INFO ] %s ", scope), log.LstdFlags|log.Lshortfile)).
		WithWarn(log.New(out, fmt.Sprintf("[WARN ] %s ", scope), log.LstdFlags|log.Lshortfile)).
		WithError(log.New(out, fmt.Sprintf("[ERROR] %s ", scope), log.LstdFlags|log.Lshortfile))
}
